                ...
                foreach($teasers as $k=>$teaser){
                   // if($k && $k=='type') continue;
                    ...
                    if($teaser['type']=='cat'){
                        ...
                        $returnarray['items'][]=array('pos'=>(int)$i, 'apectFill'=>1, 'type' => 'cat', 'id'=> (int) $teaser['id'], 'title'=> $item['title'], 'thumb'=>$this->nh_relativeToAbsolutePath($item['img']), 'cat_id'=>(int) $teaser['id'], 'post_ts'=>0, 'post_date'=>0);
                        $i++;
                        continue;
                    }
                    if($teaser['type']=='webview'){
                        ...
                        $returnarray['items'][]=array('pos'=>(int)$i, 'apectFill'=>1, 'type' => 'webview', 'id'=> (int) $teaser['id'], 'item_id'=>-3, 'title'=> $teaser['title'], 'thumb'=>$teaser['img'], 'cat_id'=>0, 'post_ts'=>0, 'post_date'=>0, 'url'=>$teaser['url']);
                        $i++;
                        continue;
                    }
                    if($teaser['type']=='favorites' || $teaser['type']=='search'){
                        ...
                        $teaser['title'] =  preg_replace_callback("/(&#[0-9]+;)/", function($m) { return mb_convert_encoding($m[1], "UTF-8", "HTML-ENTITIES"); },$teaser['title']);
                        $teaser['title'] = html_entity_decode($teaser['title']);
                        $returnarray['items'][]=array('pos'=>(int)$i, 'apectFill'=>1, 'type' => $teaser['type'], 'id'=> (int) $teaser['id'], 'item_id'=>-3, 'title'=> $teaser['title'], 'thumb'=>$teaser['img'], 'cat_id'=>0, 'post_ts'=>0, 'post_date'=>0);
                        $i++;
                        continue;
                    }
                    if(isset($teaser['customfilter'])){
                        ...
                        $returnarray['items'][]=array('pos'=>(int)$i, 'apectFill'=>1, 'type' => $teaser['type'], 'id'=> (int) $teaser['id'], 'item_id'=>null, 'title'=> $teaser['title'], 'thumb'=>$teaser['img'], 'cat_id'=>0, 'post_ts'=>0, 'post_date'=>0);
                        $i++;
                        continue;
                    }
